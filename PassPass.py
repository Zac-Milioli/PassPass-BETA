...
class PassPass:
    def __init__(self, root: ctk.CTk):
        self.root = root
        self.passpass_color_theme = '#1414b8'
        self.primary = 'white'
        ...
        self.root.config(bg=self.passpass_color_theme)
        self.top = ctk.CTkFrame(self.root, fg_color=self.passpass_color_theme, bg_color=self.passpass_color_theme)
        self.top.pack(pady=10, fill='both')

        credit = ctk.CTkLabel(self.top, text='Zac Milioli', font=credit_font, text_color=self.primary, fg_color=self.passpass_color_theme, bg_color=self.passpass_color_theme)
        credit.pack(pady=2)

        title = ctk.CTkLabel(self.top, text='PassPass', font=title_font, text_color=self.primary, fg_color=self.passpass_color_theme, bg_color=self.passpass_color_theme)
        title.pack(pady=2)

        # criar frame completo do PassPass e incluir o botão de ajuda
        self.mainscreen = ctk.CTkFrame(self.root, fg_color=self.passpass_color_theme, bg_color=self.passpass_color_theme)
        self.mainscreen.pack(fill='both', expand=True)

        self.entry1 = ctk.CTkEntry(self.mainscreen, placeholder_text='Entrada 1', placeholder_text_color=self.primary, font=elements_font, fg_color=self.secondary, bg_color=self.passpass_color_theme, width=200, border_color=self.primary, text_color=self.primary)
        self.entry1.pack(pady=10)        
        self.entry2 = ctk.CTkEntry(self.mainscreen, placeholder_text='Entrada 2', placeholder_text_color=self.primary, font=elements_font, fg_color=self.secondary, bg_color=self.passpass_color_theme, width=200, border_color=self.primary, text_color=self.primary)
        self.entry2.pack(pady=10)        
        self.entry3 = ctk.CTkEntry(self.mainscreen, placeholder_text='Entrada 3', placeholder_text_color=self.primary, font=elements_font, fg_color=self.secondary, bg_color=self.passpass_color_theme, width=200, border_color=self.primary, text_color=self.primary)
        self.entry3.pack(pady=10)        

        self.generator = ctk.CTkButton(self.mainscreen, text='🔒', font=passpass_icon_font, text_color=self.secondary, fg_color=self.primary, corner_radius=50, width=60, height=80, border_color=self.primary, command=self.generate_password)
        self.generator.pack(pady=20)

        self.output = ctk.CTkEntry(self.mainscreen, placeholder_text='', text_color=self.primary, font=elements_font, fg_color=self.secondary, bg_color=self.passpass_color_theme, width=200, border_color=self.primary, state='disabled')
        self.output.pack(pady=2)

        self.copy_clipboard = ctk.CTkButton(self.mainscreen, text='📎', width=80, height=30, font=elements_font, text_color=self.secondary, fg_color=self.primary, border_color=self.primary, command=self.copy_to_clipboard)
        ...
        self.helpscreen = ctk.CTkFrame(self.root, fg_color=self.passpass_color_theme, corner_radius=0)
        descricao_ajuda = '''
Bem vindo ao PassPass!

Seu uso é simples e direto:
1. Insira algo na primeira entrada
2. Insira algo na segunda entrada
3. Insira algo na terceira entrada
4. Pressione o botão com cadeado "🔒"
E pronto! Sua senha foi 
construída e pode ser copiada 
clicando no botão com clipe "📎"


Dicas para aumentar a segurança
--------------------------------------------------
Coloque em alguma das entradas
uma senha pessoal que você sempre
usa.
Insira o lugar onde você usará
a senha gerada em alguma das
entradas, assim fica mais fácil
de lembrar suas combinações!
'''
    ...
    def change_colors(self):
        ...
        primaria = self.change_primary_entry.get()
        secundaria = self.change_secondary_entry.get()
        if primaria != '':
            self.primary = primaria
        if secundaria != '':
            self.secondary = secundaria
        if tema != '':
            self.passpass_color_theme = tema
